...
ap.add_argument(
    "--no-normalization",
    help="Do not normalize the traversability matrix",
    action="store_true",
    default=False,
)
ap.add_argument(
    "--fixed-cut-threshold",
    help="Use a fixed cut threshold for region cutting",
    type=float,
    default=None,
)
ap.add_argument(
...
def main_experiment():
    ...
    w_path = os.path.join(
        root, "weights", COLOR, "all" if args.outliers else "no outliers"
    )
