...
for train_index, test_index in cross_val.split(X):
    ...
    if not os.path.exists(W_file) or args.overwrite:
        ...
        if running in colab: toolbox.visualize_training(tfcn.history, test_index + 1, L_path)
        else: toolbox.visualize_training(tfcn.history, test_index + 1, L_path, block=False)
    else:
        tfcn.load(wp=W_file)
    ...
    def optimize():
        ...
        def func(c, r=8):
            ...
            print('c: %.3f, p: %.3f, f+: %.3f, f-: %.3f, output: %.3f' % (c, score_mean, frdr, irdr, output))
