...
if args.extra: ex = 'extra'
else: ex = ''

W_path = os.path.join(root, 'weights', WEIGHTS_DIR, 'all' if args.outliers else 'no outliers')
...
if args.extra:
    ...
    for imagename in sorted(os.listdir(X_path)):
        if imagename.endswith('.jpg'):
            ...
            if args.rgb: x_colored = x
            else: x_colored = toolbox.imread(os.path.join(X_path, imagename), color='rgb')
                

            if args.grayscale: x = np.expand_dims(x, 2)
            ...
            y = tfcn.predict([x])
            y = np.squeeze(y[0])
            y = skimage.transform.resize(y, (y.shape[0]/r__, y.shape[1]/r__), anti_aliasing=True)
            ...
            cv.imwrite(os.path.join(output_path__, 'maps', 'tfcn-output-%s.jpg' % (imagename[:-4])), 255*y)

            T = y

            keypoints_image = toolbox.imread(os.path.join(KP_path, imagename), color='rgb')
            ...
            output_path = os.path.join(output_path__, 'paths/%s/%s/%s' % (COLOR, 'all' if args.outliers else 'no outliers', imagename[:-4]))
            ...
            for counter, (s, t) in enumerate(itertools.combinations(keypoints, 2)):
                ...
                path_image = toolbox.draw_path(x_colored, path, found=found, color=fontColor)
    ...
    for train_index, test_index in cross_val.split(X):
        ...
        if args.grayscale: x_test = np.expand_dims(x_test, 3)
        ...
        output_path = os.path.join(output_path__, 'paths/%s/%s/%s' % (COLOR, 'all' if args.outliers else 'no outliers', 'aerial%02d' % (test_index[0]+1)))
        if not os.path.exists(output_path):
            os.makedirs(output_path)
