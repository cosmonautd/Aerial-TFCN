...
def imread(path, color='rgb'):
    ...
    """
    if color == 'grayscale': return cv.cvtColor(cv.imread(path), cv.COLOR_BGR2GRAY)
    elif color == 'rgb': return cv.cvtColor(cv.imread(path), cv.COLOR_BGR2RGB)
    elif color == 'hsv': return cv.cvtColor(cv.imread(path), cv.COLOR_BGR2HSV)
    elif color == 'rgbh':
...
def grid_list(image, r):
    ...
    """
    if len(image.shape) == 2: height, width = image.shape
    elif len(image.shape) == 3: height, width, _ = image.shape
