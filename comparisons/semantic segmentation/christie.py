...
output_file = 'data-christie.json'
...
def show_images(images, block=True):
    ...
    if block: plot()
    else: multiprocessing.Process(target=plot).start()

segmentation_time = np.array([
    15148340,
    144194785,
    41917580,
    3025772,
...
def main_experiment():
    ...
    # for i in range(len(selected)):
        ...
        segmentation_path = os.path.join(root, 'results_ale', 'Result%d' % (i+1), 'Crf')
        ...
        segmentation = toolbox.imread(os.path.join(segmentation_path, selected[i][:-4] + '.png'), color='rgb')
        ...
        for counter in range(len(combinations)):
            ...
            img = toolbox.imread(os.path.join(dataset_path, selected[i][:-4] + '.jpg'), color='rgb')
            ...
            p_path = os.path.join(output_path, 'paths', 'christie', 'aerial%02d' % (i+1), 'positive')
            if not os.path.exists(p_path): os.makedirs(p_path)
            toolbox.save_image(os.path.join(p_path, 'path-%d.jpg' % (counter+1)), [path_image])
        ...
        for counter in range(len(combinations)):
            ...
            p_path = os.path.join(output_path, 'paths', 'christie', 'aerial%02d' % (i+1), 'negative')
            if not os.path.exists(p_path): os.makedirs(p_path)
            toolbox.save_image(os.path.join(p_path, 'path-%d.jpg' % (counter+1)), [path_image])
