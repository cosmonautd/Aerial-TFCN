...
def main_experiment():
    ...
    # for i in range(len(selected)):
        ...
        for counter in range(len(combinations)):
            ...
            p_path = os.path.join(output_path, 'paths', 'christie_fcn_resnet50', 'aerial%02d' % (i+1), 'positive')
        ...
        for counter in range(len(combinations)):
            ...
            p_path = os.path.join(output_path, 'paths', 'christie_fcn_resnet50', 'aerial%02d' % (i+1), 'negative')
            if not os.path.exists(p_path): os.makedirs(p_path)
            toolbox.save_image(os.path.join(p_path, 'path-%d.jpg' % (counter+1)), [path_image])
