...
if args.outliers: rgb_classes.append([254,  77,   0]) # 6 Sand
...
if not os.path.exists(os.path.join(inputs_path, 'inputs.pth')):
    ...
    for img in sorted(os.listdir(inputs_path)):
        image = cv2.cvtColor(cv2.imread(os.path.join(inputs_path, img)), cv2.COLOR_BGR2RGB)
...
if not os.path.exists(os.path.join(outputs_path, 'outputs.pth')):
    ...
    for lab in sorted(os.listdir(outputs_path)):
        label = cv2.cvtColor(cv2.imread(os.path.join(outputs_path, lab)), cv2.COLOR_BGR2RGB)
...
for n, (i, o) in enumerate(zip(inputs, outputs)):
    if n + 1 == 8 and not args.outliers: continue
    if n == test_index: testset.append((i, o))
    else: trainset.append((i, o))
...
if not os.path.exists(w_path):
    ...
    for epoch in range(100):
        ...
        print('[Map: %2d, Epoch: %3d] Training Loss: %.3f' % \
