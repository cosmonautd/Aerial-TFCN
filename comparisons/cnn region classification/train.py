...
ap.add_argument(
    "--use", type=int, default=8, required=False, help="Select how many images to use"
)
args = ap.parse_args()
    ...
    if imagename.endswith(".jpg"):
        ...
        for j, class__ in enumerate(classes):
            ...
            if os.path.exists(c_path):
                c = toolbox.imread(c_path, color="grayscale")
            else:
...
for train_index, test_index in cross_val.split(X_img):
    ...
    for j, class__ in enumerate(classes):
        ...
        print()
        print(
            "Class %s present: %.3f (+/- %.3f)"
            % (classes[j], np.mean(present), np.std(present))
        )
        print(
            "Class %s not present: %.3f (+/- %.3f)"
            % (classes[j], np.mean(not_present), np.std(not_present))
        )
    ...
    np.savetxt(
        os.path.join(W_path, "thresholds-%d.txt" % (test_index + 1)),
        thresholds,
        fmt="%f",
    )
