        ...
        for i, row in enumerate(tmatrix):
            for j, element in enumerate(row):
                ...
                G.add_node(
                    index,
                    pos=[j, i],
                    inv_traversability=float("inf")
                    if tmatrix[i][j] == 0
                    else (100 * (1 / tmatrix[i][j])) ** 2,
                ...
                if not G.nodes[u]["cut"]:
                    edges.append(
                        (
                            ...
                            u,
                            {
                                "weight": G.nodes[v]["inv_traversability"]
                                + G.nodes[u]["inv_traversability"]
                            },
                        )
                    )
        ...
        try:
            ...
            if opts.get("smooth") is True:
                ...
                interpolator = scipy.interpolate.interp1d(
                    distance, X, kind="slinear", axis=0
                )
                ...
                distance = numpy.insert(distance, 0, 0) / distance[-1]
                splines = [
                    scipy.interpolate.UnivariateSpline(distance, coords, k=2)
                    for coords in X.T
                ]
